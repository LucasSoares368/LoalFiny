...
interface IndicadoresInteligentesProps {
  userId: string;
  hideValues?: boolean;
}

export function IndicadoresInteligentes({ userId, hideValues = false }: IndicadoresInteligentesProps) {
  ...
  const loadIndicators = useCallback(async () => {
    ...
    try {
      ...
      if (isPro) {
        ...
        if (protection >= 100) {
          status = "healthy";
          statusLabel = "Proteção completa";
        } else if (protection >= 80) {
          status = "healthy";
          statusLabel = "Quase protegido";
        } else if (protection >= 50) {
          status = "warning";
      ...
      if (isProPlus) {
        ...
        const variableIncome = allIncome
          .filter((t) => t.income_type === "variable")
      ...
      if (isProPlus) {
        ...
        const netRecent = (recent ?? []).reduce(
          (s, t) => s + (t.type === "income" ? Number(t.amount) : -Number(t.amount)),
          0,
        );
        const netOlder = (older ?? []).reduce(
          (s, t) => s + (t.type === "income" ? Number(t.amount) : -Number(t.amount)),
          0,
        );
    ...
    return (
      <UpgradePrompt
        ...
        benefits={[
          ...
          "Dependência de renda variável (Business)",
        ...
        {isPro && !isProPlus && (
          <div className="pt-2 border-t">
            <p className="text-xs text-muted-foreground text-center">
              Faça upgrade para <span className="font-semibold text-primary">Business</span> e desbloqueie o indicador
              de dependência de renda variável.
            </p>
          </div>
        )}
      </CardContent>
