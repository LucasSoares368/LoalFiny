...
import { ArrowLeft, Upload, CheckCircle2, Loader2, Database, FileSpreadsheet, Trash2, FileText, X, AlertCircle, Image, Camera, RefreshCw, Lock } from "lucide-react";
...
const ImportData = () => {
  ...
  const { canImportData, loading: planLoading } = useUserPlan();
  ...
  const startCamera = async (mode: "user" | "environment" = facingMode) => {
    try {
      // If camera is already open with a different mode, stop it first
      ...
      setTimeout(async () => {
        try {
          ...
          console.error("Inner camera error:", err);
          toast.error("Erro ao iniciar transmissão de vídeo");
    ...
    return (
      <AppLayout title="Importar Dados">
        <div className="mx-auto w-full max-w-3xl space-y-6">
          ...
          <UpgradePrompt 
            feature="Importar Dados"
            description="Economize tempo importando seus extratos bancários, PDFs e planilhas automaticamente usando nossa tecnologia de IA."
            requiredPlan="pro"
          />
        </div>
      </AppLayout>
