...
async function sendWhatsAppMessage(
  ...
  try {
    // Get centralized Evolution API config
    ...
    // Fallback to database config if env vars are missing
    if (!apiUrl || !apiKey || !instanceName) {
      ...
      if (dbConfig?.api_url && dbConfig?.api_key && dbConfig?.instance_name) {
    ...
    if (!apiUrl || !apiKey || !instanceName) {
      console.error("Evolution API not configured in environment or database");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (targetUserId) {
      // System call must present the service-role key as bearer to prevent abuse
      const authHeader = req.headers.get("Authorization") || "";
      const token = authHeader.replace("Bearer ", "").trim();
      if (token !== supabaseServiceKey) {
        return new Response(
          JSON.stringify({ error: "Forbidden: targetUserId requires service role" }),
