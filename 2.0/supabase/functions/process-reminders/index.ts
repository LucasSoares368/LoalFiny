...
async function generateDebtReminder(supabase: any, userId: string, reminder: Reminder): Promise<string | null> {
  if (!reminder.reference_id) {
    return (
      `💳 *Lembrete: ${reminder.title}*\n\n` +
      `${reminder.message ? `${reminder.message}\n\n` : ""}` +
      `${reminder.day_of_month ? `Vence dia ${reminder.day_of_month}` : ""}\n\n` +
      `_Financeiro Pro - Seu assistente financeiro_`
    );
  }

  const { data: debt } = await supabase.from("debts").select("*").eq("id", reminder.reference_id).single();
...
async function generateDailySummary(supabase: any, userId: string): Promise<string> {
  const now = new Date();
  ...
  const personalBankBalance = (personalBanks || []).reduce(
    (acc: number, b: any) => acc + parseFloat(b.current_balance),
    0,
  );
  const businessBankBalance = (businessBanks || []).reduce(
    (acc: number, b: any) => acc + parseFloat(b.current_balance),
    0,
  );
  ...
  const personalDebtTotal = (personalDebts || []).reduce(
    (acc: number, d: any) => acc + parseFloat(d.current_balance),
    0,
  );
  const businessDebtTotal = (businessDebts || []).reduce(
    (acc: number, d: any) => acc + parseFloat(d.current_balance),
    0,
  );
  ...
  if (personalTxnsList.length > 0) {
    ...
    personalTxnsList.slice(0, 5).forEach((txn) => {
      message += `${txn}\n`;
    });
  ...
  if (businessTxnsList.length > 0) {
    message += `📊 *Movimentações de hoje:*\n`;
    businessTxnsList.slice(0, 5).forEach((txn) => {
      message += `${txn}\n`;
    });
  ...
  message += `_Financeiro Pro - Seu assistente financeiro_`;
...
async function generateMonthlySummary(supabase: any, userId: string): Promise<string> {
  const now = new Date();
  ...
  let prevPersonalIncome = 0,
    prevPersonalExpenses = 0;
  ...
  let prevBusinessIncome = 0,
    prevBusinessExpenses = 0;
  ...
  const personalTopCategories = Object.entries(personalCategoryExpenses)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
  const businessTopCategories = Object.entries(businessCategoryExpenses)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5);
...
async function smartCheckGoals(supabase: any, userId: string): Promise<string | null> {
  ...
  for (const g of goals) {
    ...
    if (remaining < 0) {
      alerts.push(
        `• *${g.icon || "🎯"} ${g.name}*: Prazo vencido há ${Math.abs(remaining)} dias! Progresso: ${actual.toFixed(0)}%`,
      );
    } else if (expected - actual > 15) {
      alerts.push(
        `• *${g.icon || "🎯"} ${g.name}*: ${actual.toFixed(0)}% (deveria ~${expected.toFixed(0)}%). Faltam ${formatCurrency(Number(g.target_amount) - Number(g.current_amount))} em ${remaining} dias.`,
      );
    }
  }
...
async function smartCheckRecurring(supabase: any, userId: string): Promise<string | null> {
  ...
  for (const [, d] of catMap) {
    if (d.previous > 50) {
      ...
      if (pct > 30)
        increases.push(
          `• *${d.name}*: ${formatCurrency(d.previous)} → ${formatCurrency(d.current)} (+${pct.toFixed(0)}%)`,
        );
    }
  }
...
async function smartCheckLowBalance(supabase: any, userId: string): Promise<string | null> {
  ...
  if (total < monthlyAvg * 0.5 && monthlyAvg > 0) {
    const lowBanks = banks
      .filter((b: any) => Number(b.current_balance) < 100)
...
async function smartCheckSavingsTip(supabase: any, userId: string): Promise<string | null> {
  ...
  tip += `_Financeiro Pro - Seu assistente financeiro_`;
  return tip;
}
...
async function smartCheckDebtAlert(supabase: any, userId: string): Promise<string | null> {
  const { data: debts } = await supabase
    .from("debts")
    .select("name, current_balance, due_day, creditor")
    .eq("user_id", userId)
    .eq("status", "active");
  ...
  for (const d of debts) {
    ...
    if (daysUntil <= 5 && daysUntil >= 0) {
      upcoming.push(
        `• *${d.name}*${d.creditor ? ` (${d.creditor})` : ""}: ${formatCurrency(Number(d.current_balance))} - vence ${daysUntil === 0 ? "HOJE" : `em ${daysUntil} dia${daysUntil > 1 ? "s" : ""}`}`,
      );
    }
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const reminder of reminders || []) {
      ...
      // Update last_sent_at
      await supabase.from("reminders").update({ last_sent_at: new Date().toISOString() }).eq("id", reminder.id);
