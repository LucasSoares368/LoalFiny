...
const buildLatestTransactionReply = (transaction: TransactionContext | null, requestedProfile: ProfileType | null) => {
  if (!transaction) {
  ...
  const profileLabel = transaction.profile === "personal" ? "perfil pessoal" : "perfil empresarial";
...
serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      messages.some(
        (message) =>
          !message || typeof message.content !== "string" || !["system", "user", "assistant"].includes(message.role),
      )
    ) {
    ...
    if (configError || !openAiConfig?.api_key) {
      ...
      return jsonResponse({ error: "OpenAI API Key não configurada pelo administrador" }, 500);
    }
    ...
    const [
      ...
      adminClient
        .from("banks")
        .select(
          "id, name, current_balance, initial_balance, account_type, is_active, profile_type, agency, account_number, color",
        )
        .eq("user_id", user.id)
      ...
      adminClient
        .from("debts")
        .select(
          "id, name, creditor, total_amount, current_balance, interest_rate, minimum_payment, due_day, start_date, status, notes, profile_type",
        )
        .eq("user_id", user.id)
        .order("created_at", { ascending: false }),
      adminClient
      ...
      adminClient
        .from("reminders")
        .select(
          "id, title, reminder_type, is_active, next_send_at, day_of_month, day_of_week, time_of_day, days_before",
        )
        .eq("user_id", user.id)
        .order("created_at", { ascending: false }),
      adminClient
        ...
        .eq("user_id", user.id)
        .order("created_at", { ascending: false }),
      adminClient.from("user_balance").select("available_balance, updated_at").eq("user_id", user.id).maybeSingle(),
    ]);
    ...
    const latestPersonalTransaction = transactions.find((transaction) => transaction.profile === "personal") ?? null;
    const latestBusinessTransaction = transactions.find((transaction) => transaction.profile === "business") ?? null;
    ...
    const contextPayload = {
      ...
      summary: {
        ...
        consolidated_active_bank_balance: activeBanks.reduce((sum, bank) => sum + Number(bank.current_balance), 0),
        active_bank_balance_personal: personalBanks.reduce((sum, bank) => sum + Number(bank.current_balance), 0),
        active_bank_balance_business: businessBanks.reduce((sum, bank) => sum + Number(bank.current_balance), 0),
      },
      latest_records: {
        ...
        latest_debt_payment: debtPayments[0]
          ? {
              ...debtPayments[0],
      ...
      balances: {
        available_balance:
          userBalance?.available_balance === null || userBalance?.available_balance === undefined
            ? null
      ...
      debts: debts.map((debt) => ({
        ...
        minimum_payment: debt.minimum_payment === null ? null : Number(debt.minimum_payment),
      })),
      ...
      emergency_goal: emergencyGoal
        ? {
            ...emergencyGoal,
            target_amount: emergencyGoal.target_amount === null ? null : Number(emergencyGoal.target_amount),
            current_amount: emergencyGoal.current_amount === null ? null : Number(emergencyGoal.current_amount),
          }
        : null,
    ...
    const systemPrompt = `Você é o Assistente Financeiro IA do Financeiro Pro. Responda sempre em português do Brasil.
...
${JSON.stringify(contextPayload)}`;
    ...
    if (!response.ok) {
      if (response.status === 429) {
        return jsonResponse({ error: "Muitas requisições. Tente novamente em alguns segundos." }, 429);
      }
